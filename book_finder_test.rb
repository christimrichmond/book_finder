...
class Testbookfinder < Minitest::Test 
	...
	def test_assert_Blink_returns_isbn_number
		assert_equal("9780316010665", book_name("Blink"))
	end

    def test_assert_Defying_Hitler_returns_isbn_number
		assert_equal("0312421133", book_name("Defying Hitler"))
	end

    def test_assert_Keep_the_Home_Fires_Burning_returns_isbn_number
		assert_equal("1510059202", book_name("Keep the Home Fires Burning - Part One: Spitfire Down!"))
	end

    def test_assert_Ruby_Cookbook_title_returns_isbn_number
		assert_equal("1449373712", book_name("Ruby Cookbook: Recipes for Object-Oriented Scripting"))
	end

    def test_assert_Test_Driven_Development_title_returns_isbn_number
		assert_equal("1484226372", book_name("Test Driven Development in Ruby"))
	end
